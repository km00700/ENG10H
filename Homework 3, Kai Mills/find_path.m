function [positions, depth] = find_path(position, max_tree_depth, ast_coords)
    ...
    %Returns the postions from the recursive search for the largest depth.
        ...
        [center_path, center_depth] = find_path(position + [0, 1], max_tree_depth - 1, ast_coords);
        center_path_length = center_depth;
       
        if(position(1) > 0)
            [left_path, left_depth] = find_path(position + [-1, 1], max_tree_depth - 1, ast_coords);
            left_path_length = left_depth;
        end
        
        if(position(1) < 20)
            [right_path, right_depth] = find_path(position + [1, 1], max_tree_depth - 1, ast_coords);
        ...
        positions = [position; [longest_path(:,1), longest_path(:,2)]];
        depth = longest_depth;
    end
end